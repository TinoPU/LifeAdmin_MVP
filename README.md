...
1) Create bot via `@BotFather`, set `TELEGRAM_BOT_TOKEN` in `.env`.
2) Deploy your server and set webhook:
```bash
curl -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/setWebhook" \
  -H "Content-Type: application/json" \
  -d '{"url":"https://your-domain.com/api/telegram"}'
```
3) Optionally restrict a single user:
```env
ALLOWED_TELEGRAM_ID=123456789
```
4) Local test helper:
...
1) Configure Business Cloud API creds in `.env` (`WA_URL`, `WA_PHONE_ID`, `WA_TOKEN`).
2) Expose webhook URL `/api/webhooks` and verify token as needed.
3) Incoming messages are processed, user fetched/created, and passed into AgentManager.

## Supabase Schema (conceptual)

Tables:
- `users`: id, wa_id, telegram_id, name, user_timezone, language, preferences
- `tasks`: id, user_id, name, task_description, due_date, status, source
- `reminders`: id, task_id, user_id, reminder_time, status

You may add:
- `agent_artifacts` (optional, for referential grounding) – store last agent outputs with IDs

## Composio Integration

Composio is wired via `src/tools/composioClient.ts`:
- Initializes the SDK with `COMPOSIO_API_KEY` and Anthropic provider
- Agents can fetch available tools, call LLM with tools, then execute returned tool calls through Composio

Email agent workflow:
1) Check `connectedAccounts` for Gmail; if none, initiate and return redirect URL
2) Fetch tools for `GMAIL`
3) Call Anthropic with those tools (via shared `callAgent` helper) and receive tool calls
4) Execute tools with `composio.provider.handleToolCalls`
